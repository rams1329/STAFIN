...
@Repository
public class JobFunctionRepositoryImpl implements JobFunctionRepositoryCustom {

    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public Page<JobFunction> findAllWithFilters(
        ...
        CriteriaQuery<JobFunction> query = cb.createQuery(JobFunction.class);
        Root<JobFunction> root = query.from(JobFunction.class);
        ...
        List<JobFunction> jobFunctions = entityManager.createQuery(query)
        ...
        Root<JobFunction> countRoot = countQuery.from(JobFunction.class);
        countQuery.select(cb.count(countRoot));
        countQuery.where(predicates.toArray(new Predicate[0]));
        Long total = entityManager.createQuery(countQuery).getSingleResult();

        return new PageImpl<>(jobFunctions, pageable, total);
    }
} 